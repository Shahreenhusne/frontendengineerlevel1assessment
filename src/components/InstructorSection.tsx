...
const InstructorSection = ({sections}:InstructorSectionProps ) => {
    const instructorSection = sections.find((section) => section.type === "instructors");
   if (!instructorSection || instructorSection.values.length === 0) return null;


   const instructor = instructorSection?.values?.[0];

  return (
    <section id="instructors" className="mb-7 xs:bg-[#EEF2F4] xs:pt-2">
    <div className="pt-4 pb-2 bg-white">
      <h2 className="mb-4 text-xl font-semibold md:text-2xl">Course instructor</h2>
      <div className="flex items-center md:rounded-md md:border-gray-100 md:p-5 shadow-md">
        <div>
          <div className="rounded-full overflow-hidden opacity-100">
             <Image
        ...
        <div className="flex-1 ml-4">
          <h3 className="text-lg">
            <a className="flex items-center hover:text-green" href={`/skills/instructors/${instructor.slug}/`}>
             {instructor.name}
